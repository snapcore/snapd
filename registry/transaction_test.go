...
func (s *transactionTestSuite) TestSet(c *C) {
	...
	tx, err := registry.NewTransaction(witness.read, witness.write, reg)
	c.Assert(err, IsNil)
...
func (s *transactionTestSuite) TestCommittedIncludesPreviousCommit(c *C) {
	...
	reg := newRegistry(c, registry.NewJSONSchema())
	txOne, err := registry.NewTransaction(readBag, writeBag, reg)
	c.Assert(err, IsNil)

	txTwo, err := registry.NewTransaction(readBag, writeBag, reg)
	c.Assert(err, IsNil)
...
func (s *transactionTestSuite) TestTransactionReadsIsolated(c *C) {
	...
	reg := newRegistry(c, registry.NewJSONSchema())
	tx, err := registry.NewTransaction(readBag, writeBag, reg)
	c.Assert(err, IsNil)
