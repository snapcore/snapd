...
static void sc_mkdir_and_mount_and_bind_wsl_gpu(const char *rootfs_dir,
					const char *src_dir,
					const char *dst_dir)
{
	...
	// Bind mount the binary WSL2 GPU driver into $tgt_dir (i.e. /var/lib/snapd/lib/gl).
...
void sc_mount_wsl2_gpu_driver(const char *rootfs_dir)
{
	...
	(void)sc_set_effective_identity(old);
	sc_mkdir_and_mount_and_bind_wsl_gpu(rootfs_dir, SC_HOST_WSL_DIR,
					    SC_WSL_GPU_DIR);
}
