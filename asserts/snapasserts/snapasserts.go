...
func DeriveComponentSideInfoFromDigestAndSize(resName, snapName, snapID string, compPath, snapSHA3_384 string, resSize uint64, db Finder) (*snap.ComponentSideInfo, error) {
	// get relevant assertions and reconstruct metadata
	headers := map[string]string{
		"snap-id":           snapID,
...
func FetchResourceRevisionAssertion(f asserts.Fetcher, si *snap.SideInfo, compName, hash, provenance string) error {
	ref := &asserts.Ref{
		Type:       asserts.SnapResourceRevisionType,
		PrimaryKey: []string{si.SnapID, compName, hash},
	}
	if provenance != "" {
		ref.PrimaryKey = append(ref.PrimaryKey, provenance)
	}
...
func FetchResourcePairAssertion(f asserts.Fetcher, si *snap.SideInfo, compName string, compRev snap.Revision, provenance string) error {
	ref := &asserts.Ref{
		Type:       asserts.SnapResourcePairType,
		PrimaryKey: []string{si.SnapID, compName, compRev.String(), si.Revision.String()},
	}
	if provenance != "" {
		ref.PrimaryKey = append(ref.PrimaryKey, provenance)
	}
