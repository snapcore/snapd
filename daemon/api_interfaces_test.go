...
func (s *interfacesSuite) TestInterfacesAllDefaultDocURL(c *check.C) {
	...
	if result, ok := body["result"].([]interface{}); ok {
		c.Assert(len(result), check.Equals, 190, check.Commentf("New interface detected, please ensure it is documented at https://snapcraft.io/docs/<name>-interface and increment the count"))
