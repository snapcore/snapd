...
func (s *snapsSuite) TestPostRemoveComponents(c *check.C) {
	...
	var t *state.Task
	defer daemon.MockSnapstateRemoveComponents(func(st *state.State, snapName string, compNames []string) ([]*state.TaskSet, error) {
...
func (s *snapsSuite) TestPostComponentsManyRemoveCompsAndSnap(c *check.C) {
	...
	c.Check(rspe.Status, check.Equals, 400)
	c.Check(rspe.Message, testutil.Contains,
		`cannot remove "snap1", "bar": cannot mix components and snap when removing "snap1"`)
}
