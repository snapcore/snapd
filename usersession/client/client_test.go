...
func (s *clientSuite) TestServicesRestartOrReloadFailure(c *C) {
  ...
  "result": {
    ...
    "value": {
      "restart-errors": {
        "service2.service": "failed to reload or restart"
      }
    }
  }
}`))
	...
	c.Check(failure0, DeepEquals, client.ServiceFailure{
		Uid:     42,
		Service: "service2.service",
		Error:   "failed to reload or restart",
	})
	c.Check(failure1, DeepEquals, client.ServiceFailure{
		Uid:     1000,
		Service: "service2.service",
...
func (s *clientSuite) TestServiceStatus(c *C) {
  ...
  "result": [{
	...
	si, err := s.cli.ServiceStatus(context.Background(), []string{"snap.foo.service"})
	c.Assert(err, IsNil)
	c.Check(si, DeepEquals, map[int][]client.UserServiceUnitStatus{
		42: {
			{
				Daemon:           "notify",
				Id:               "snap.foo.service",
...
func (s *clientSuite) TestServiceStatusError(c *C) {
	s.handler = http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(500)
		w.Write([]byte(`{
...
func (s *clientSuite) TestServiceStatusWrongResultType(c *C) {
  ...
  "result": ["a", "list"]
	...
	si, err := s.cli.ServiceStatus(context.Background(), []string{"snap.foo.service"})
	c.Check(si, DeepEquals, map[int][]client.UserServiceUnitStatus{})
	c.Check(err, ErrorMatches, `json: cannot unmarshal string into Go value of type client.UserServiceUnitStatus`)
}
