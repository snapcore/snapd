...
func serviceReloadOrRestart(inst *serviceInstruction, sysd systemd.Systemd) Response {
	...
	return SyncResponse(&resp{
		...
		Status: 500,
		Result: &errorResult{
			Message: "some user services failed to reload or restart",
			Kind:    errorKindServiceControl,
			Value: map[string]interface{}{
				"restart-errors": restartErrors,
			},
...
func unitStatusToClientUnitStatus(units []*systemd.UnitStatus) []*client.UserServiceUnitStatus {
	var results []*client.UserServiceUnitStatus
	for _, u := range units {
		results = append(results, &client.UserServiceUnitStatus{
			Daemon:           u.Daemon,
			Id:               u.Id,
			Name:             u.Name,
			Names:            u.Names,
...
func serviceStatus(c *Command, r *http.Request) Response {
	...
	stss, err := sysd.Status(services)
	if err != nil {
		return SyncResponse(&resp{
			Type:   ResponseTypeError,
			Status: 500,
			Result: &errorResult{
				Message: fmt.Sprintf("cannot retrieve status for service: %v", err),
