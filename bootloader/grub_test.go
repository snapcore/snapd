...
func (s *grubTestSuite) TestConstructShimEfiLoadOption(c *C) {
	...
	description, assetPath, optionalData, err := ubl.EfiLoadOptionParameters([]string{"bootx64.efi", "boot:fbx64.efi", "ubuntu:shimx64.efi"})
	c.Assert(err, IsNil)
...
func (s *grubTestSuite) TestConstructShimEfiLoadOptionOldBootchain(c *C) {
	...
	description, assetPath, optionalData, err := ubl.EfiLoadOptionParameters([]string{"bootx64.efi"})
	c.Assert(err, IsNil)
	c.Assert(description, Equals, "ubuntu")
