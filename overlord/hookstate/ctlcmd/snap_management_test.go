...
func (s *installSuite) testMngmtCommand(c *C, cmd string) {
	...
	switch cmd {
	case "install":
		ctlcmd.MockSnapstateInstallComponentsFunc(func(st *state.State, comps []string) (
			[]*state.TaskSet, error) {
			c.Check(comps, DeepEquals, []string{"comp1", "comp2"})
			var ts state.TaskSet
			ts.AddTask(task)
			return []*state.TaskSet{&ts}, nil
		})
	case "remove":
		ctlcmd.MockSnapstateRemoveComponentsFunc(func(st *state.State, comps []string) (
			[]*state.TaskSet, error) {
			c.Check(comps, DeepEquals, []string{"comp1", "comp2"})
			var ts state.TaskSet
			ts.AddTask(task)
