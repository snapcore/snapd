...
func (c *failCommand) Execute(args []string) error {
	...
	if ctx.IsEphemeral() || ctx.HookName() != "change-registry" {
		return errors.New(i18n.G("cannot use `snapctl fail` outside of a \"change-registry\" hook"))
	}
