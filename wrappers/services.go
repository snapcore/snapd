...
func usersToUids(users []string) (map[int]string, error) {
	...
	if strutil.ListContains(users, "users") {
		// means all users, return an empty list of uids
		return uids, nil
	}
...
func newUserServiceClient(users []string, inter Interacter) (*userServiceClient, error) {
...
func daemonScopeCoveredByServiceScope(dscope snap.DaemonScope, scope ServiceScope) bool {
	switch scope {
...
func StartServices(apps []*snap.AppInfo, disabledSvcs []string, flags *StartServicesFlags, inter Interacter, tm timings.Measurer) (err error) {
	...
	cli, err := newUserServiceClient(flags.Users, inter)
	if err != nil {
		return err
	}
	...
	for _, app := range apps {
		...
		if !daemonScopeCoveredByServiceScope(app.DaemonScope, flags.Scope) {
			continue
		}
...
// XXX: Currently this only retrieves enable-state for the system services, should
// we extend this to cover all user-services as well?
func ServicesEnableState(s *snap.Info, inter Interacter) (map[string]bool, error) {
	sysd := systemd.New(systemd.SystemMode, inter)
	sts, _, err := internal.QueryServiceStatusMany(s.Services(), sysd)
	if err != nil {
		return nil, err
	}
...
// in the snap.
// XXX: Currently this only retrieves state for the system services, should
// we extend this to cover all user-services as well?
