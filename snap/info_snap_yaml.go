...
func infoFromSnapYaml(yamlData []byte, strk *scopedTracker) (*Info, error) {
	...
	if snap.Type() != TypeSnapd && y.SnapdInfo.UbuntuCoreTracks == nil && hasSnapdInfoKey(yamlData) {
...
// whatever its value. yaml.v2 does not invoke field UnmarshalYAML methods for
// explicit null values, so this is only needed to distinguish null from omitted.
