...
`, "group", "mygroup")
	...
	grp, err := user.LookupGroupFromGetent(0, "mygroup")
	c.Assert(err, IsNil)
	c.Assert(grp, NotNil)
...
func (s *getentSuite) TestLookupGroupByNameError(c *C) {
	_, err := user.LookupGroupFromGetent(0, "mygroup")
	c.Assert(err, NotNil)
}
...
func (s *getentSuite) TestLookupGroupByGid(c *C) {
	s.mockGetentOutput(c, `mygroup:x:60000:myuser,someuser
`, "group", "60000")

	grp, err := user.LookupGroupFromGetent(2, "60000")
	c.Assert(err, IsNil)
	c.Assert(grp, NotNil)
	c.Check(grp.Name, Equals, "mygroup")
	c.Check(grp.Gid, Equals, "60000")
...
`, "group")
	...
	grp, err := user.LookupGroupFromGetent(0, "1mygroup")
	c.Assert(err, IsNil)
	c.Assert(grp, NotNil)
...
`, "passwd", "johndoe")
	...
	usr, err := user.LookupUserFromGetent(0, "johndoe")
	c.Assert(err, IsNil)
	c.Assert(usr, NotNil)
...
`, "passwd", "60000")
	...
	usr, err := user.LookupUserFromGetent(2, "60000")
	c.Assert(err, IsNil)
	c.Assert(usr, NotNil)
	c.Check(usr.Username, Equals, "johndoe")
	c.Check(usr.Uid, Equals, "60000")
...
`, "passwd")
	...
	usr, err := user.LookupUserFromGetent(0, "1johndoe")
	c.Assert(err, IsNil)
	c.Assert(usr, NotNil)
