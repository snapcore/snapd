...
func (tr *tree20) seedSnapComponents(sn *SeedSnap) []internal.ComponentOpt {
	compOpts := make([]internal.ComponentOpt, len(sn.Components))
	for i, comp := range sn.Components {
		unassertedComp := ""
		if sn.Info.ID() == "" {
			unassertedComp = filepath.Base(comp.Path)
		}
		compOpts[i] = internal.ComponentOpt{
